...
- **Depot-by-depot rollout gives every regional manager a low-risk on-ramp** and a chance to raise operational concerns before the platform is fleet-wide (see `ARC-001-STRAT-v1.0`).
- **Finance is engaged on realised savings, not just projected ones** — the economic case in `ARC-001-SOBC-v1.0` is revisited after each rollout phase with actuals.
